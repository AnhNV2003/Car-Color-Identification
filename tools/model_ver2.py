...
class ColorAwareConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.target_hw = (8, 8)  # avoid AdaptiveAvgPool2d for ONNX dynamic H/W
    ...
    def forward(self, x):
        ...
        color_stats = F.interpolate(x, size=self.target_hw, mode="bilinear", align_corners=False)
        ...
        color_features = color_stats.mean(dim=(-2, -1), keepdim=True)
        color_features = F.interpolate(color_features, size=out.shape[2:])
...
class MobileViTCo(nn.Module):
    def __init__(self, image_size=128, num_classes=4, variant='XXS'):
        ...
        if variant == 'XS':
            ...
            channels = [8, 16, 32, 48, 64]           # ~50% reduction from XS
            transformer_dims = [32, 48, 64]          # ~50% reduction from XS
            depths = [1, 1, 2]                       # Reduced transformer depth
            mlp_dims = [64, 96, 128] 
    ...
    def forward(self, x):
        ...
        x = x.mean(dim=(-2, -1), keepdim=True)  # 4x4 -> 1x1
        x = torch.flatten(x, 1)
